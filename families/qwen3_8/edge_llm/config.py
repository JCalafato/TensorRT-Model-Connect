...
"""Explicit Qwen38 paired-build inputs; no shared execution-mode contract."""
...
class BuildExecutionInputs:
    ...
    Qwen38 validates these inputs without fetching or inferring companions.
    """
...
@dataclass(frozen=True)
class Qwen38BuildRequest(BuildRequest):
    """Ordinary build inputs plus an explicitly requested Qwen38 execution recipe."""

    execution: BuildExecutionInputs | None = None

    def __post_init__(self) -> None:
        super().__post_init__()
        if self.family != "qwen3_8":
            raise ValueError("Qwen38BuildRequest requires the qwen3_8 family")
...
def with_execution(request: BuildRequest, execution: BuildExecutionInputs) -> Qwen38BuildRequest:
    """Preserve ordinary request fields and callback identity."""
    return Qwen38BuildRequest(
