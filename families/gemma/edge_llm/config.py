...
"""Explicit Gemma paired-build inputs; no shared execution-mode contract."""
...
class BuildExecutionInputs:
    ...
    Gemma validates these inputs without fetching or inferring companions.
    """
...
@dataclass(frozen=True)
class GemmaBuildRequest(BuildRequest):
    """Ordinary build inputs plus an explicitly requested Gemma execution recipe."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.family != "gemma":
            raise ValueError("GemmaBuildRequest requires the gemma family")
